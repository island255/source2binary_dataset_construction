...
def add_binary_function_info(address_function_dict, mapping_relation):
    ...
    for file_line_address in mapping_relation:
        ...
        try:
            ...
            print("cannot find the binary function of this address {}".format(address))
...
def add_source_info_for_line(args):
    line, project_dir, sub_paths, source_entities = args
    ...
    line_number = line[1]
    line_number_refer_entity = get_line_number_refer_entity(project_dir, line_number, source_file_path,
                                                            source_entities)
...
def add_source_function_information(project_dir, source2binary_mapping_detail, source_entities):
    ...
    for line in source2binary_mapping_detail:
        args.append([line, project_dir, sub_paths, source_entities])

    with Pool(processes=32) as pool:
        _partial_func = partial(add_source_info_for_line)
        pool.map(_partial_func, args)
...
def get_binary2source_entity_mapping(source2binary_mapping_full):
    ...
    for mapping_line in source2binary_mapping_full:
        ...
        if mapping_line[1] == "0" and mapping_line[2] is None:
            continue

        if [mapping_line[0], mapping_line[1], mapping_line[2]] not in binary2source_entity_mapping_line_dict[
            mapping_line[-2]]:
            binary2source_entity_mapping_line_dict[mapping_line[-2]].append(
                [mapping_line[0], mapping_line[1], mapping_line[2]])

        if [mapping_line[0], mapping_line[2]] not in binary2source_entity_mapping_simple_dict[mapping_line[-2]]:
            binary2source_entity_mapping_simple_dict[mapping_line[-2]].append([mapping_line[0], mapping_line[2]])
    return binary2source_entity_mapping_line_dict, binary2source_entity_mapping_simple_dict, unresolved_binary_address
