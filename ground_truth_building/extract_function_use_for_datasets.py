...
def construct_ground_truth(understand_tool, understand_extract_script, understand_python,
    ...
    call_graph_path = "/outside_dataset/function_use"
    if not os.path.exists(call_graph_path):
        os.makedirs(call_graph_path)

    source_project = os.path.basename(source_dir)
    understand_source_entities_file = os.path.join(call_graph_path, source_project + ".json")
...
def main():
    ...
    understand_extract_script = "/root/scitools/bin/linux64/Python/use_understand_to_extract_use.py"
