                ...
                if lexemes_e.ent().parent() and lexemes_e.ent().parent() == entity and \
                    ...
                    for ref in refs:
                        if ref.ent().parent():
                            ...
                            # if "Macro" not in call_entity_kind:
                            #     continue
                            macro_expansion_example = ref.macroexpansion()
                            call_entity_value = ref.ent().value()
                            call_file = ref.ent().parent().relname()
                            ref_line = ref.line()
                            entity_ref["use"].append(
                                {"file": call_file, "entity": call_entity, "value": call_entity_value,
                                 "kind": call_entity_kind, "line_number": ref_line})

                    sub_entity_dict[sub_entity.simplename()] = {"kind": kind.longname(), "type": sub_entity.type(),
