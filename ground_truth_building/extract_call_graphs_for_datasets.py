...
def use_understand_extract_entities(understand_tool, source_dir, understand_python, understand_extract_script,
    ...
    if os.path.exists(source_project_und) and os.path.exists(understand_source_entities_file):
        return
    if not os.path.exists(source_project_und):
...
def construct_ground_truth(understand_tool, understand_extract_script, understand_python,
    ...
    call_graph_path = "/outside_dataset/call_graphs"
    if not os.path.exists(call_graph_path):
        os.makedirs(call_graph_path)

    source_project = os.path.basename(source_dir)
    understand_source_entities_file = os.path.join(call_graph_path, source_project + "_call_graph.json")
...
def main():
    ...
    understand_extract_script = "/root/scitools/bin/linux64/Python/use_understand_to_extract_cg.py"
